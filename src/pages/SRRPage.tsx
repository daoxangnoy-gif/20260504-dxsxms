      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col flex-1 min-h-0 overflow-hidden">
        <div className="px-4 pt-2 border-b border-border bg-card flex items-center gap-2">
          <TabsList className="h-9">
            ...
            <TabsTrigger value="report2" className="text-xs gap-1.5">
              <BarChart3 className="w-3.5 h-3.5" /> Report 2
            </TabsTrigger>
          </TabsList>
        ...
        <TabsContent value="read-cal" className="flex-1 flex flex-col mt-0 min-h-0 data-[state=inactive]:hidden">
          ...
          <div className="flex items-center gap-1.5 px-3 py-2 bg-card border-b border-border flex-wrap">
              ...
              <>
                ...
                >
                  <MultiSelect
                    compact
                    label="SPC Name"
                    options={preSpcOptions.length > 0 ? preSpcOptions : spcOptions}
                    selected={selectedSpcForCal}
                    onChange={(v) => { setSelectedSpcForCal(v); setDataReady(false); }}
                  />
                  <MultiSelect
                    compact
                    label="Order Day"
                    ...
                    onChange={(v) => { setOrderDayCal(v); setDataReady(false); }}
                    searchable={false}
                  />
                  <MultiSelect
                    compact
                    ...
                    onChange={(v) => { setVendorFilterCal(v); setDataReady(false); }}
                  />
                  <MultiSelect
                    compact
                    label="Item Type"
            ...
            {/* === Import / Vendor mode: SPC / Type Store / Store Name dropdowns === */}
            {importMode !== "filter" && (
              <>
                <MultiSelect
                  compact
            ...
            {!dataReady && !loading && !dataLoadingMsg && (
              <div className="flex items-center gap-1.5 px-2 h-7 rounded-md bg-muted/50 border border-border text-muted-foreground text-xs font-medium">
                ...
                เลือก Filter แล้วกดเตรียมข้อมูล
              </div>
            )}
            {!dataReady && !loading && dataLoadingMsg && (
