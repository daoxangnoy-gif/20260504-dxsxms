...
interface B2BRow {
  ...
  // From data master
  sku_code: string;
  main_barcode: string;       // unit barcode (packing_size_qty=1)
...
async function fetchByIdsParallel<T>(
  ...
  for (let i = 0; i < chunks.length; i += concurrency) {
    ...
    const results = await Promise.all(wave.map(async (slice) => {
      const { data, error } = await (supabase as any).from(table).select(selectCols).in(inColumn, slice);
...
// ============================================================
const stateRef: {
  importedRaw: { key: string; product_name_imp: string; qty: number }[];
  rows: B2BRow[];
  skips: SkipItem[];
  activeTab: string;
...
export default function SRROrderB2BPage() {
  ...
  const parsePasteImport = () => {
    ...
    const items: { key: string; product_name_imp: string; qty: number }[] = [];
    for (const line of lines) {
      ...
      items.push({ key, product_name_imp: name, qty });
    }
  ...
  const downloadTemplate = () => {
    const ws = XLSX.utils.aoa_to_sheet([
      ["barcode/skucode", "product_name_imp", "qty"],
      ["8851234567890", "ตัวอย่างชื่อสินค้า", 5],
      ["SKU-00123", "ตัวอย่าง 2", 12],
    ]);
  ...
  const handleGetDate = async () => {
    ...
    try {
      ...
      const seenSku = new Set<string>(); // distinct skucode
      ...
      for (const imp of importedRaw) {
        ...
        if (seenSku.has(sku)) {
          newSkips.push({ imp_key: imp.key, product_name_imp: imp.product_name_imp, reason: "SKU ซ้ำ", qty: imp.qty });
          continue;
        }
        seenSku.add(sku);
  ...
  // ============================================================
  // SAVE SO/PO — create BOTH an SO doc and a PO doc from current rows
  //   - SO doc: copy of all rows (SO tab applies its own toggle filter)
  //   - PO doc: only rows where stock_dc < qty
  // ============================================================
  const doSaveSOPO = () => {
    if (rows.length === 0) { toast({ title: "ไม่มีข้อมูล", variant: "destructive" }); return; }
    if (!poCustomer) { toast({ title: "กรุณาเลือก Customer", variant: "destructive" }); return; }
    const ts = tsCompact();
    const cust = poCustomer;
    const baseRows: B2BRow[] = JSON.parse(JSON.stringify(rows));
    const poRows = baseRows.filter(r => r.stock_dc < r.qty);

    const newDocs: SavedB2BDoc[] = [];
    // SO doc — always created (SO tab still has its own row toggle)
    newDocs.push({
      id: `b2b-${ts}-so-${Math.random().toString(36).slice(2, 6)}`,
      filename: `${ts}-${cust}-SO`,
      name: cust, created_at: new Date().toISOString(),
      doc_type: "so",
      customer_name: cust,
    ...
    // PO doc — only if there are rows with stock_dc < qty
    if (poRows.length > 0) {
      newDocs.push({
        id: `b2b-${ts}-po-${Math.random().toString(36).slice(2, 6)}`,
        filename: `${ts}-${cust}-PO`,
        name: cust, created_at: new Date().toISOString(),
        doc_type: "po",
        customer_name: cust,
    ...
    toast({
      ...
      description: `SO doc · ${poRows.length > 0 ? "PO doc · " : "(ไม่มีรายการ Stock DC < qty สำหรับ PO) · "}${newDocs.length} เอกสาร`,
    });
  };

  // SAVE RO — store-based. Saves only to RO Doc (not PO). Multi-store: one doc per selected store.
  const doSaveRO = () => {
    if (rows.length === 0) { toast({ title: "ไม่มีข้อมูล", variant: "destructive" }); return; }
    if (roStores.length === 0) { toast({ title: "กรุณาเลือก Store อย่างน้อย 1 รายการ", variant: "destructive" }); return; }
    const ts = tsCompact();
    const newDocs: SavedB2BDoc[] = roStores.map((s, i) => {
      const cust = s.store_name;
      const baseRows: B2BRow[] = JSON.parse(JSON.stringify(rows));
      return {
    ...
    <div className="flex flex-col h-full overflow-hidden bg-background">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-card">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <div className="w-px h-5 bg-border mx-1" />
          <Button size="sm" variant="outline" onClick={() => setSavePOOpen(true)} disabled={rows.length === 0} className="text-xs h-8">
            <Save className="w-3.5 h-3.5 mr-1" /> Save SO/PO
          </Button>
          <Button size="sm" variant="outline" onClick={() => setSaveROOpen(true)} disabled={rows.length === 0} className="text-xs h-8">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden">
        <TabsList className="mx-4 mt-2 grid grid-cols-4 max-w-lg">
          <TabsTrigger value="data" className="text-xs">Data View</TabsTrigger>
          <TabsTrigger value="so" className="text-xs">SO Doc</TabsTrigger>
          <TabsTrigger value="po" className="text-xs">PO Doc</TabsTrigger>
          <TabsTrigger value="ro" className="text-xs">RO Doc</TabsTrigger>
        </TabsList>
...
function SODocList({ docs, onChange, allDocs, onPreview }: {
  ...
  const grouped = useMemo(() => {
    const m = new Map<string, Map<string, SavedB2BDoc[]>>();
    for (const d of filteredEligible) {
      const stamp = d.filename.split("-")[0];
      const cust = d.customer_name || "";
      if (!m.has(stamp)) m.set(stamp, new Map());
      const cm = m.get(stamp)!;
  ...
  const buildSORows = (d: SavedB2BDoc): any[] => {
    ...
    for (let i = 0; i < elig.length; i += itemsPerSO) {
      const chunk = elig.slice(i, i + itemsPerSO);
      chunk.forEach((r, idx) => {
        if (isRO) {
          out.push({
    ...
    <div className="space-y-2">
      ...
      {eligible.length === 0 ? (
        ...
        <>
          ...
          {[...grouped.entries()].sort((a, b) => b[0].localeCompare(a[0])).map(([stamp, custMap]) => {
            const isExp = expanded.has(stamp);
            return (
              <div key={stamp} className="border border-border rounded-lg overflow-hidden">
                <button onClick={() => toggleStamp(stamp)} className="w-full flex items-center gap-2 px-3 py-2 bg-muted/50 hover:bg-muted">
...
function PODocList({ docs, onChange, allDocs, onPreview }: {
  ...
  // Grouping by stamp → vendor
  const grouped = useMemo(() => {
    const m = new Map<string, Map<string, SavedB2BDoc[]>>();
    for (const d of eligible) {
      const stamp = d.filename.split("-")[0];
      // For tree display, group docs by their primary vendor — but doc may have multiple vendors;
      // here we list by doc and split inside on export.
      const vKey = "(multi)";
      if (!m.has(stamp)) m.set(stamp, new Map());
      const vm = m.get(stamp)!;
  ...
  return (
    <div className="space-y-2">
      ...
      {[...grouped.entries()].sort((a, b) => b[0].localeCompare(a[0])).map(([stamp, vMap]) => {
        const isExp = expanded.has(stamp);
